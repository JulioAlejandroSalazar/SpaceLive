...
--Mi secuencia
CREATE SEQUENCE seq_detalle_bonificaciones
START WITH 1
INCREMENT BY 1
NOCACHE
NOCYCLE;
...
-- Desafio 1 - Eliminación de sinónimos si ya existen
BEGIN
    EXECUTE IMMEDIATE 'DROP SYNONYM trabajador_new';
EXCEPTION
    WHEN OTHERS THEN
        NULL;
END;
...
BEGIN
    EXECUTE IMMEDIATE 'DROP SYNONYM bono_antiguedad_new';
EXCEPTION
    WHEN OTHERS THEN
        NULL;
END;
...
BEGIN
    EXECUTE IMMEDIATE 'DROP SYNONYM tickets_concierto_new';
EXCEPTION
    WHEN OTHERS THEN
        NULL;
END;
...
-- Desafio 2 - Eliminación de sinónimos si ya existen
BEGIN
    EXECUTE IMMEDIATE 'DROP SYNONYM bono_escolar_new';
EXCEPTION
    WHEN OTHERS THEN
        NULL;
END;
...
BEGIN
    EXECUTE IMMEDIATE 'DROP SYNONYM asignacion_familiar_new';
EXCEPTION
    WHEN OTHERS THEN
        NULL;
END;
...
-- Desafio 3 - Eliminación de índices si ya existen
BEGIN
    EXECUTE IMMEDIATE 'DROP INDEX idx_apmaterno';
EXCEPTION
    WHEN OTHERS THEN
        NULL;
END;
...
BEGIN
    EXECUTE IMMEDIATE 'DROP INDEX idx_cod_isapre';
EXCEPTION
    WHEN OTHERS THEN
        NULL;
END;
...
BEGIN
    EXECUTE IMMEDIATE 'DROP INDEX idx_apmaterno_cod_isapre';
EXCEPTION
    WHEN OTHERS THEN
        NULL;
END;
...
-- Desafio 2 - Eliminación de vista si ya existe
BEGIN
...
CREATE SYNONYM trabajador_new FOR ADMIN.trabajador;
CREATE SYNONYM bono_antiguedad_new FOR ADMIN.bono_antiguedad;
CREATE SYNONYM tickets_concierto_new FOR ADMIN.tickets_concierto;

-- Calculo de antiguedad laboral con MONTHS_BETWEEN y NonEquiJoin
SELECT 
    t.numrut || '-' || t.dvrut AS rut_completo,
    t.nombre || ' ' || t.appaterno || ' ' || t.apmaterno AS nombre_completo,
    t.sueldo_base,
    b.porcentaje / 100 AS porcentaje_antiguedad,
    t.sueldo_base * (1 + b.porcentaje / 100) AS sueldo_simulado_antiguedad
FROM trabajador t
JOIN bono_antiguedad b
    ON MONTHS_BETWEEN(SYSDATE, t.fecing) / 12 BETWEEN b.limite_inferior AND b.limite_superior
WHERE t.sueldo_base < 50
  AND t.cod_isapre > 4;

-- Calculo de bonificaciones por ticket
SELECT 
    t.numrut || '-' || t.dvrut AS rut_completo,
    NVL(tc.monto_ticket, 0) AS monto_ticket,
    CASE
        WHEN tc.monto_ticket > 100000 THEN tc.monto_ticket * 0.07
        WHEN tc.monto_ticket > 50000 THEN tc.monto_ticket * 0.05
        ELSE 0
    END AS bonif_x_ticket
FROM trabajador t
LEFT JOIN tickets_concierto tc ON t.numrut = tc.numrut_t;

-- Unificación de datos e inserción en DETALLE_BONIFICACIONES_TRABAJADOR
INSERT INTO detalle_bonificaciones_trabajador (
    num, rut, nombre_trabajador, sueldo_base, num_ticket, bonif_x_ticket, direccion, sistema_salud, simulacion_antiguedad
)
SELECT 
    seq_detalle_bonificaciones.NEXTVAL,
    t.numrut || '-' || t.dvrut,
    t.nombre || ' ' || t.appaterno || ' ' || t.apmaterno,
    t.sueldo_base,
    NVL(tc.monto_ticket, 0) AS monto_ticket,
    CASE
        WHEN NVL(tc.monto_ticket, 0) > 100000 THEN NVL(tc.monto_ticket, 0) * 0.07
        WHEN NVL(tc.monto_ticket, 0) > 50000 THEN NVL(tc.monto_ticket, 0) * 0.05
        ELSE 0
    END AS bonif_x_ticket,
    t.direccion,
    i.nombre_isapre,
    t.sueldo_base * (1 + b.porcentaje / 100)
FROM trabajador t
LEFT JOIN tickets_concierto tc ON t.numrut = tc.numrut_t
JOIN bono_antiguedad b
    ON MONTHS_BETWEEN(SYSDATE, t.fecnac) / 12 < 50
    AND b.limite_inferior <= MONTHS_BETWEEN(SYSDATE, t.fecing) / 12 
    AND b.limite_superior >= MONTHS_BETWEEN(SYSDATE, t.fecing) / 12
JOIN isapre i ON t.cod_isapre = i.cod_isapre
WHERE b.porcentaje > 4
AND t.cod_isapre > 4
AND (tc.monto_ticket IS NOT NULL OR tc.monto_ticket IS NULL);


-- Desafio 2
-- Creación de sinónimos privados
CREATE SYNONYM bono_escolar_new FOR espacio.bono_escolar;
CREATE SYNONYM asignacion_familiar_new FOR espacio.asignacion_familiar;

-- Creación de la vista V_AUMENTOS_ESTUDIOS
CREATE VIEW v_aumentos_estudios AS
SELECT 
    t.numrut || '-' || t.dvrut AS rut_completo,
    INITCAP(t.nombre) || ' ' || INITCAP(t.appaterno) || ' ' || INITCAP(t.apmaterno) AS nombre_completo,
    e.sigla AS nivel_educativo,
    (e.porc_bono / 100) porcentaje_educacion,
    t.sueldo_base,
    (t.sueldo_base * (1 + e.porc_bono / 100)) sueldo_simulado
FROM trabajador t
JOIN bono_escolar e ON t.id_escolaridad_t = e.id_escolar
WHERE t.id_categoria_t = 1
   OR t.numrut IN (
       SELECT af.numrut_t
...
-- Desafio 3
-- Creación del índice por apellido materno
CREATE INDEX idx_apmaterno ON trabajador (apmaterno);

-- Creación del índice para evitar Full Scan en TRABAJADOR
CREATE INDEX idx_cod_isapre ON trabajador (cod_isapre);
CREATE INDEX idx_apmaterno_cod_isapre ON trabajador (apmaterno, cod_isapre);
